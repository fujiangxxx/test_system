    ...
    export default {
        ...
        data(){
            return {
                datas:[],
                color:"red",
                stuClass:this.$store.state.updateUclassed==""?JSON.parse(sessionStorage.getItem('stuLogin')).class:this.$store.state.updateUclassed,
            }
        },
        mounted(){
            var that=this;
            fetch("/api/stuSet/selectClass").then(function (e) {
                return e.json();
            }).then((e)=>{
                e.forEach(function (item,index) {
                    item.color="#000";
                    if(item.name==that.stuClass){
                        item.color="blue"
                    }
                })
                this.datas=e;
