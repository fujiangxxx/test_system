    ...
    mysql.query("select * from zuti where id="+zid,function (err,result) {
        ...
        mysql.query(`select * from test where id in (${tis}) order by field (id,${tis})`,function (err1,result1) {
            ...
            mysql.query(`select * from result where sid=${sid} and zid=${zid}`,function (err2,result2) {
                ...
                var errors=[];
                //答题结果
                var jiandaResult=result2[0].jianda.split("|");
                ...
                for(var i=0;i<result1.length;i++){
                    ...
                    if(result1[i].typeid==3){
                       ...
                       //处理考生的简答题结果
                       for(var z=0;z<jiandaResult.length;z++){
                            var arr1=jiandaResult[z].split(":");
                           if(result1[i].id==arr1[0]){
                               result1[i].jiandaStus=arr1[1];
                           }
                       }
                        result1[i].ok="yes";
...
router.get("/daanInfo",function (req,res) {
    ...
    var id=req.query.id;
    mysql.query("select * from test where id="+id,function (err,result) {
        if(result[0].typeid==1){
            result[0].daan= parseInt(result[0].daan)+1;
        }else if(result[0].typeid==2){
            result[0].daan=result[0].daan.replace(/\|/g,"").split("");
            for(var i=0;i<result[0].daan.length;i++){
                if(!isNaN(result[0].daan[i])||result[0].daan[i]=="0"){

                    result[0].daan[i]=parseInt(result[0].daan[i])+1;

                }
            }
            result[0].daan.join(",");
            console.log(typeof result[0].daan);
        }else if(result[0].typeid==3){
            result[0].daan=result[0].daan.replace(/\n/g,'');
            console.log(result[0].daan);
        }
